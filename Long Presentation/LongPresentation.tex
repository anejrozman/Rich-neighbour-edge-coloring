...
\section{Algorithms}
    ...
    \subsection{Integer Programming}
        ...
        \begin{center}
            \begin{tikzpicture}[every node/.style={draw,circle, minimum size=4mm, style={text opacity=0}}]
                \node (A) at (0,0) {$1$};
                \node (B) at (2,0) {$2$};
                ...
                % levo
                \draw[line width=0.7pt, label=1] (A.west) -- ++(-2,1);
                \draw[line width=0.7pt] (A.west) -- ++(-2,-1);

                %desno
                \draw[line width=0.7pt] (B.east) -- ++(2,1);
                \draw[line width=0.7pt] (B.east) -- ++(2,-1);
...
\section{Random Search}
    ...
    Written below is the algorithm \texttt{tweak} for modifying graphs. First it selects two random edges and deletes them. Then it generates a random number $p$ from $[0, 1)$ and based on the value of $p$ it adds back two different edges in one of two possible ways that preserve the regularity of the graph. There is a chance that the newly formed graph is not connected, so we check for that and if it is not, we restart the process until we get a connected graph.\\
...
\section{Checking the coloring} 
    In order to make sure that our ILP implentation is without fault, we also implemented the functions \texttt{checkColoring} and \texttt{checkRichness} which check if our program always returns a proper coloring, and if the coloring is actually a rich-neighbor edge coloring.\\
    \textcolor{white}{i\\ i\\ i\\}
    \pagebreak
    
    \texttt{checkColoring} takes a graph $G$ and the output $colors$ of our ILP. It iterates over\* each vertex in the graph, checks its neighbors and returns false if a color repeats on multiple of that edges.\\ %of the edges that include it,  repeats. \\
    ...
    \texttt{checkRichness} takes a graph $G$ and the output $richEdges$ of our ILP. It iterates over each edge in the graph, checks its neighbors and returns false if it doesn't find a rich neighboring edge for each edge.\\
